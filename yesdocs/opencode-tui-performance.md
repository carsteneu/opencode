...
| ---------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------- |
| Currently deployed OpenCode build        | `1.18.18-patched.104`, upstream merge commit `bbecbbc241aa28ff41298218cf664c2c6d66e43c`     | Local `working` build                                                       |
| Current public OpenCode prerelease       | `1.18.18-patched.104`, upstream merge commit `bbecbbc241aa28ff41298218cf664c2c6d66e43c`     | [tag](https://github.com/carsteneu/opencode/tree/1.18.18-patched.104)       |
| Previous public OpenCode prerelease      | `1.18.16-patched.103`, save-directory commit `311f0776bd46746768ff5c6ca257af24ccfc9761`    | [tag](https://github.com/carsteneu/opencode/tree/1.18.16-patched.103)       |
...
| OpenTUI renderer in `.98` through `.104` | OpenTUI 0.5.1 plus ported patches, commit `568db413e7bc3a110981d2e54ddb7ebb8e906075`       | Tag `opencode-1.18.16-patched.98`                                           |
| Current Linux binary                     | `.104`, x86_64, SHA-256 `e0804131f83748223e3234020cf04ba02541d35da37c4bec6b76ecaa77b2c948` | Local deployed artifact                                                     |
| Previous deployed Linux binary           | `.103`, x86_64, SHA-256 `5a2866276559ed6d5f47426f672ad2fa50c4f33b16043f5c9e14ff486ca8c79d` | Backup `opencode-1.18.16-patched.103-before-104.bak`                        |
...
The locally deployed `.102` build replaces the cell snapshot path with bounded native viewport residency. An
image that remains resident stays at native terminal quality while the session streams, and a URI change keeps
the previous sharp image visible until the replacement has loaded. There is no coarse intermediate image. A
maximum of two native images may coexist, cold native decoding is limited to images that actually overlap the
viewport, and existing residents receive a two-row hysteresis plus a 0.75-viewport retention band. This avoids
rapid release and reload cycles around scroll boundaries. Actual `scrollTop` movement determines which adjacent
source is preferred.

Encoded originals use a separate exact-URI LRU with at most eight entries and 32 MiB. Active native sources hold
leases, so the LRU cannot evict their bytes. At most two source loaders run concurrently, adjacent prefetch is
limited to two candidates after a 100 ms settle, shared requests are coalesced, and abandoned work is cancelled
when its final consumer leaves. Evicted old sources can be fetched again when revisited, which keeps history
memory bounded. The existing HTTPS, address, redirect, MIME, timeout, encoded-byte, and decoded-pixel guards
remain in force. Inline previews allow four million pixels per image, while the explicit dialog allows eight
million pixels. Images outside residency create no empty `ImageRenderable` objects.

The locally deployed `.103` build adds a directory prompt to the image save action. Its default is an `img`
directory at the active project root. For Git projects, that root is the active worktree; non-Git sessions fall
back to the active instance directory and then the startup directory. The prompt accepts absolute paths,
project-relative paths, and `~` or `~/...` home paths. It does not evaluate shell variables or commands, and the
image filename continues to be generated, sanitized, collision-safe, and derived from the actual encoded
format. Cancelling before confirmation performs no filesystem work. A load or write error leaves the prompt
open for correction, and closing it while a save is active aborts the operation and its temporary file.
...
The `.102` image residency change passed the complete TUI suite with 258 tests passed, 1 existing skip, and no
failures, plus the TUI and OpenCode package typechecks. Focused tests cover exact viewport selection, scroll
direction, boundary overlap, hysteresis, a hard two-resident cap, atomic native replacement, independent native
images, stable object identity while streaming, source request coalescing, per-consumer cancellation, clean
retry, pinned LRU entries, byte accounting, and the hard two-loader concurrency limit. Targeted linting reported
no errors; its 23 warnings were pre-existing warnings in the larger touched files and tests.

The full embedded Web UI and Linux x86_64 build completed from commit
`f77d7522cb509b0ed6f4e2e2ebeeabf107ae3420`. The pinned OpenTUI Core, Solid, and native hashes matched before
packaging, and the build passed its version smoke test. The atomically installed binary reports
`1.18.16-patched.102`, is byte-identical to the build artifact, and has SHA-256
`f3142ca748ad7681552b63dc8139db97c6ca82a3498a87eb1ce954d786eea835`. The previous `.101` binary is retained
unchanged as `opencode-1.18.16-patched.101-before-102.bak`, with its original SHA-256
`f70cb221cb93cf0f848ce80ccb3946f7109e247e41b3ce8b8cb8c37a8b13822e`.

The `.103` save-directory change passed the complete TUI suite with 262 tests passed, 1 existing skip, and no
failures, plus the TUI and OpenCode package typechecks. Fourteen focused tests cover the default project `img`
directory, active worktree and fallback selection, absolute, relative, and home-relative path resolution, lack
of shell expansion, empty and invalid input rejection, cancellation without filesystem work, configurable
keybindings, selected-image preservation, exact original bytes, collision handling, symlink protection,
parallel saves, and abort cleanup. Two independent reviews found no implementation blocker.

The full embedded Web UI and Linux x86_64 build completed from commit
`311f0776bd46746768ff5c6ca257af24ccfc9761`. The pinned OpenTUI Core, Solid, and native hashes matched before
and after packaging, and the build passed its version smoke test. The atomically installed binary reports
`1.18.16-patched.103`, is byte-identical to the build artifact, and has SHA-256
`5a2866276559ed6d5f47426f672ad2fa50c4f33b16043f5c9e14ff486ca8c79d`. The previous `.102` binary is retained
unchanged as `opencode-1.18.16-patched.102-before-103.bak`, with its original SHA-256
`f3142ca748ad7681552b63dc8139db97c6ca82a3498a87eb1ce954d786eea835`.

The `.104` integration merges the 20 upstream commits missing after the `1.18.16` base and advances all
workspace package versions to `1.18.18`. The upstream changes add Merge Gateway reasoning support, pass reasoning
effort through the xAI, Mistral, and Groq adapters, select the Kimi prompt by provider, refine compaction prompts
and plugin context placement, and update DeepSeek V4 Pro metadata. Upstream changes no TUI source, image,
windowing, streaming, or partial-render implementation in this integration. The merge keeps OpenTUI 0.5.1, the
direct TUI `marked` dependency, compatible plugin peers, and the complete pinned fork overlay.

Validation passed the complete TUI suite with 262 tests passed, 1 existing skip, and no failures, plus the TUI
and OpenCode package typechecks. Four focused OpenCode suites covering LLM process isolation, provider
transforms, compaction, provider-cache behavior, and system prompt selection passed 480 tests with 1 existing
skip and no failures. The pinned OpenTUI source passed 295 focused Core renderer, Markdown, Code, and ScrollBox
tests plus its Solid Markdown content-update test. The Core, Solid, and native overlay hashes matched before and
after packaging.

The full embedded Web UI and Linux x86_64 build completed from merge commit
`bbecbbc241aa28ff41298218cf664c2c6d66e43c`. The build passed its version smoke test. The atomically installed
binary reports `1.18.18-patched.104`, is byte-identical to the build artifact, and has SHA-256
`e0804131f83748223e3234020cf04ba02541d35da37c4bec6b76ecaa77b2c948`. The previous `.103` binary is retained
unchanged as `opencode-1.18.16-patched.103-before-104.bak`, with its original SHA-256
`5a2866276559ed6d5f47426f672ad2fa50c4f33b16043f5c9e14ff486ca8c79d`.
...
integration, and matching native `libopentui.so`. The `.98` through `.104` builds contain the corresponding
patched OpenTUI 0.5.1 artifacts.

The `.98` through `.104` OpenCode lockfiles name the released OpenTUI 0.5.1 packages. A fresh `bun install`
therefore resolves stock OpenTUI 0.5.1, not the additional fork commits. Reproducing the deployed renderer
requires building the tagged OpenTUI fork, placing its matching JavaScript and native artifacts into the OpenCode
dependency tree, and then building OpenCode without reinstalling those dependencies.
...
The `.98` through `.104` artifact hashes are:
...
- The strongest controlled stock comparisons use OpenCode 1.18.1 and patched `.89` or `.90`. There is no
  controlled stock OpenCode 1.18.18 versus patched `.104` benchmark yet.
...
[OpenCode 1.18.18-patched.104](https://github.com/carsteneu/opencode/releases/tag/1.18.18-patched.104). The renderer
...
The `.103` OpenCode binary is preserved as the local rollback build. `.104` retains the complete performance
and stability patch set, bounded sharp native viewport residency, generic original-image downloads, and
project-rooted save directory selection while integrating the upstream OpenCode 1.18.18 changes.
