    ...
    Effect.gen(function* () {
          ...
          return yield* Effect.scoped(
            Effect.gen(function* () {
              ...
              const release = yield* Effect.fromNullishOr(
                ...
                Effect.mapError(() => new UpgradeFailedError({ stderr: "Patched release assets were not found." })),
              )
              ...
              ).pipe(Effect.mapError(() => new UpgradeFailedError({ stderr: "Patched release checksum was invalid." })))
      ...
      const result: Interface = {
        ...
        latest: Effect.fn("Installation.latest")(function* (installMethod?: Method, refresh = false) {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get(`https://api.github.com/repos/${InstallationUpdateRepository}/releases/latest`).pipe(
              HttpClientRequest.acceptJson,
            ),
          )
