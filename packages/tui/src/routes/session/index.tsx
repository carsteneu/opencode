...
export function Session() {
  ...
  // Completed assistant messages are immutable transcript entries. Keep their
  // rendered element so streaming deltas for the active message do not make
  // Solid reconcile every completed message still mounted in the viewport.
  const bufferedMessages = new Map<string, JSX.Element>()
  createRenderEffect(
    on(
      () => route.sessionID,
                  ...
                  {(message, index) => {
                    if (message.role === "assistant" && message.time.completed) {
                      ...
                      if (!revertMessageID || message.id < revertMessageID) {
                        const buffered = bufferedMessages.get(message.id)
                        ...
                        bufferedMessages.set(message.id, element)
                        return element
                      }
                    }
                    return (
