...
export const buildReplayPrompt = (input: { readonly context?: readonly string[] }) =>
  [
    "Create an anchored summary from the conversation messages above so another coding agent can continue the work.",
    ...(input.context ?? []),
    SUMMARY_TEMPLATE,
  ]
    .filter(Boolean)
